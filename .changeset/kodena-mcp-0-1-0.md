...
Initial public release of `@sawala/kodena-mcp` — a [Model Context Protocol](https://modelcontextprotocol.io) server that lets any MCP-capable AI agent (Claude Desktop, Claude Code, Cursor, …) drive the Kodena API on a user's behalf using their existing `~/.kodena/credentials`.
