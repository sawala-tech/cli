  ...
  org
    ...
    .action(async (slug: string | undefined) => {
      ...
      if (changingOrg) {
        ...
        await updateConfig(SAWALA_BRAND, {
...
async function resolveProjectForOrg(
  ...
  if (items.length === 0) {
    process.stdout.write(
      `No projects in '${orgSlug}' yet — create one, then run \`sawala project use <slug>\`.\n`,
    )
    return { slug: null, id: null }
  }
  if (items.length === 1) {
  ...
  if (!process.stdout.isTTY) {
    process.stdout.write(
      `'${orgSlug}' has ${items.length} projects — run \`sawala project use <slug>\` to pick one.\n`,
    )
    return { slug: null, id: null }
  }
  ...
  if (!picked) {
    process.stdout.write('No project selected — run `sawala project use <slug>` to set one.\n')
    return { slug: null, id: null }
  }
