...
import { browserLogin } from '@sawala/auth'
...
const TOKENS_PAGE = 'https://sawala.cloud/dashboard/org/cli-tokens'
...
export function createLoginCommand(): Command {
  return new Command('login')
    .description('Authenticate the CLI with Kodena. Opens a browser by default — no token to copy or paste.')
...
// Validate a token against GET /me and read its server-side scope. Throws with a
// clear message if the token is rejected.
async function validateToken(apiBase: string, token: string): Promise<ValidatedLogin> {
  ...
  if (!res.ok) {
    throw new Error(
      `Token rejected by ${apiBase}/me (HTTP ${res.status}). ` +
...
async function manualPaste(apiBase: string): Promise<ValidatedLogin> {
  process.stdout.write(
    `Open ${TOKENS_PAGE} in your browser, mint a token, and paste it here.\n`,
  )
...
async function finishLogin(apiBase: string, result: ValidatedLogin): Promise<void> {
  ...
  if (allowedOrgs.length === 1) {
    ...
    // Token is scoped but the API didn't return its org in /me/orgs (could be a
    // Clerk-sync hiccup). Trust the token's scope claim.
