...
import { loadContext, requireActiveOrg, type CliContext } from '../lib/resolve'
...
export function createProjectCommand(): Command {
  ...
  project
    ...
    .action(async (slug: string | undefined) => {
      ...
      await updateConfig({ activeProject: match.slug })
...
async function pickProjectInteractively(
  ...
  if (!process.stdout.isTTY) {
    throw new Error(
      'No project slug given and not running interactively. Pass one: `kodena project use <slug>`.',
    )
  }
